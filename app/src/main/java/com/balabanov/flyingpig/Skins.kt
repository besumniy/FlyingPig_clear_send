...
import com.android.billingclient.api.BillingClientStateListener as BillingClientStateListener
...
class Skins : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*mBillingClient = BillingClient.newBuilder(this).setListener(PurchasesUpdatedListener(){
            ...
            when(pig_i){
                ...
                7->{
                    /*var billingFlowParams=BillingFlowParams.newBuilder()
                        .setSkuDetails(mSkuDetailsMap.get("pig_panda")).build()
...
fun initBilling() {
        mBillingClient = BillingClient.newBuilder(this).setListener(PurchasesUpdatedListener{
            ...
            //override fun onPurchasesUpdated(responseCode:Int,  purchases: (Mutable)List<Purchase!>?) {
                //if (billingResult == BillingClient.BillingResponse.OK && purchases != null) {
    ...
    mBillingClient.startConnection(object:BillingClientStateListener {
            override fun onBillingSetupFinished(billingResponseCode:Int) {
                if (billingResponseCode == BillingClient.BillingResponse.OK) {
                    ...
                    //var purchasesList = queryPurchases(); //query for purchases

                    //if the purchase has already been made to give the goods
                    /*for (i in 0..purchasesList.size) {
                        var purchaseId = purchasesList.get(i).getSku();
